"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.generateRobotsTxt = void 0;
const policy_1 = require("../policy");
exports.generateRobotsTxt = (config) => {
    if (!config.generateRobotsTxt) {
        return null;
    }
    const { additionalSitemaps, policies } = config.robotsTxtOptions;
    const normalizedPolices = policy_1.normalizePolicy(policies);
    let content = '';
    normalizedPolices.forEach((x) => {
        content += `User-agent: ${x.userAgent}\n`;
        if (x.allow) {
            content += `${policy_1.addPolicies('Allow', x.allow)}`;
        }
        if (x.disallow) {
            content += `${policy_1.addPolicies('Disallow', x.disallow)}`;
        }
    });
    // Append host
    content += `Host: ${config.siteUrl}\n`;
    additionalSitemaps.forEach((x) => {
        content += `Sitemap: ${x}\n`;
    });
    return content;
};
